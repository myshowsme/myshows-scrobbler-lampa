...
- Внешние плееры: по возвращении засчитываются все серии плейлиста, просмотренные за сеанс,
  а не только запущенная.
...
Правится только `src/` и при пуше — сборка и деплой автоматические, устареть артефакт не может.
...
- возврат из внешнего плеера: запущенная серия закрывается своей обычной сессией, а серии,
  переключённые внутри плеера (от запущенной вперёд до последней сыгранной), отмечаются парой
  `/start`+`/stop` — промежуточные на 100 %, последняя по фактическому прогрессу и порогу.
  При прыжке назад внутри плеера засчитывается только реально доигранная серия.
